...
export const loginUser = (email, password) => async (dispatch) => {
    try {
        dispatch({ type: "LoginRequest" }); // Dispatching the login request action
        const { data } = await axios.post(
            "/api/v1/login",
            { email, password },
            { headers: { "Content-Type": "application/json" } } // Proper JSON header
        );
        dispatch({ type: "LoginSuccess", payload: data.user }); // Dispatching success with user data
    } catch (error) {
        dispatch({
            type: "LoginFailure",
            payload: error.response?.data?.message || error.message // Better error fallback
        });
    }
};
...
export const loadUser = () => async (dispatch) => {
    try {
        ...
        dispatch({ type: "LoadUserSuccess", payload: data.user }); // Dispatching success with user data
    } catch (error) {
        dispatch({
